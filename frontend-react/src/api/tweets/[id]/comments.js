...
export default function handler(req, res) {
    ...
    if (req.method === "GET") {
        ...
        return res.json(list.map(c => ({
            id: c.id, user: state.users.find(u => u.id === c.userId)?.username || "user",
    ...
    if (req.method === "POST") {
        ...
        const c = { id: state.nextCommentId++, userId: meId, text: String(text), created_at: new Date().toISOString() };
